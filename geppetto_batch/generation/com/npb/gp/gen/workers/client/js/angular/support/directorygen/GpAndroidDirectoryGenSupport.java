 ...
 * Date Created: 03/09/2015</br>
 * @since .2</p> 
 * 
 * Contains the logic to generate the directories needed to support code</br>
 * generation for AngularJS for Android</p>
 */
@Component("GpAndroidDirectoryGenSupport")
public class GpAndroidDirectoryGenSupport {
	...
	private void setup_tablet_directories() throws Exception{
		...
		Path android_tablet_root_directory_path = Paths.get(gen_worker
				.getAndroid_root_directory_path().toString() + this.file_separator
				+ tokens[0]);
		Files.createDirectory(android_tablet_root_directory_path);
		gen_worker.setAndroid_tablet_root_directory_path(android_tablet_root_directory_path);
		...
		Path app_root_directory_path = Paths.get(android_tablet_root_directory_path.toString()
											+ this.file_separator + tokens[0]);
		Files.createDirectory(app_root_directory_path);
		gen_worker.setAndroid_tablet_app_root_directory_path(app_root_directory_path);
		...
		Path lib_root_directory_path = Paths.get(android_tablet_root_directory_path.toString() + this.file_separator
				+ tokens[0]);
		Files.createDirectory(lib_root_directory_path);
		gen_worker.setAndroid_tablet_app_lib_root_directory_path(lib_root_directory_path);
	...
	private void setup_phone_directories() throws Exception{
		...
		Path android_phone_root_directory_path 
							= Paths.get(gen_worker
								.getAndroid_root_directory_path().toString()
											+ this.file_separator + tokens[0]);
		Files.createDirectory(android_phone_root_directory_path);
		gen_worker.setAndroid_phone_root_directory_path(android_phone_root_directory_path);
		...
		Path app_root_directory_path = Paths.get(android_phone_root_directory_path.toString()
											+ this.file_separator + tokens[0]);
		Files.createDirectory(app_root_directory_path);
		gen_worker.setAndroid_phone_app_root_directory_path(app_root_directory_path);
		...
		Path lib_root_directory_path = Paths.get(android_phone_root_directory_path.toString() + this.file_separator
				+ tokens[0]);
		Files.createDirectory(lib_root_directory_path);
		gen_worker.setAndroid_phone_app_lib_root_directory_path(lib_root_directory_path);
	...
	private void setup_base_os() throws Exception{
		...
		String gen_client_android_root_directory_name =  this.base_configs.get(
				GpGenConstants.GEN_CLIENT_ANDROID_DIRECTORY_NAME).getValue();
		
		tokens = gen_worker.tokenize_string(
				gen_client_android_root_directory_name, null);

		Path android_root_directory_path 
			= Paths.get(gen_worker
							.getAngular_root_directory_path().toString()
										+ this.file_separator + tokens[0]);
		
		Files.createDirectory(android_root_directory_path);
		gen_worker.setAndroid_root_directory_path(android_root_directory_path);
	...
	private void setup_standard_lib_directories(Path base_directory, String device_type) throws Exception{
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_lib_css_directory_path(app_lib_css_directory_path);	
		}else{
			gen_worker.setAndroid_tablet_app_lib_css_directory_path(app_lib_css_directory_path);
		}
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_lib_fonts_directory_path(app_lib_fonts_directory_path);
		}else{
			gen_worker.setAndroid_tablet_app_lib_fonts_directory_path(app_lib_fonts_directory_path);
		}
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_lib_js_directory_path(app_lib_js_directory_path);
		}else{
			gen_worker.setAndroid_tablet_app_lib_js_directory_path(app_lib_js_directory_path);
		}
	...
	private void setup_standard_app_directories(Path base_directory, String device_type) throws Exception{
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_css_directory_path(app_css_directory_path);	
		}else{
			gen_worker.setAndroid_tablet_app_css_directory_path(app_css_directory_path);
		}
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_fonts_directory_path(app_fonts_directory_path);	
		}else{
			gen_worker.setAndroid_tablet_app_fonts_directory_path(app_fonts_directory_path);
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_views_directory_path(app_views_directory_path);
		}else{
			gen_worker.setAndroid_tablet_app_views_directory_path(app_views_directory_path);
		}
		...
		if(device_type.equals("phone")){
			gen_worker.setAndroid_phone_app_root_js_directory_path(app_js_root_directory_path);	
		}else{
			gen_worker.setAndroid_tablet_app_root_js_directory_path(app_js_root_directory_path);
		}
		...
		if(device_type.equals("phone")){
			app_controller_directory_path = Paths.get(
					gen_worker.getAndroid_phone_app_root_js_directory_path()
									.toString() + this.file_separator + tokens[0]);
			Files.createDirectory(app_controller_directory_path);
			gen_worker.setAndroid_phone_app_controller_directory_path(app_controller_directory_path);
		}else{
			app_controller_directory_path = Paths.get(
					gen_worker.getAndroid_tablet_app_root_js_directory_path()
									.toString() + this.file_separator + tokens[0]);
			Files.createDirectory(app_controller_directory_path);
			gen_worker.setAndroid_tablet_app_controller_directory_path(app_controller_directory_path);
		}
		...
		if(device_type.equals("phone")){
			...
			gen_worker.setAndroid_phone_app_directives_directory_path(app_directives_directory_path);
		}else{
			app_directives_directory_path = Paths.get(
					gen_worker.getAndroid_tablet_app_root_js_directory_path()
									.toString() + this.file_separator + tokens[0]);
			Files.createDirectory(app_directives_directory_path);
			gen_worker.setAndroid_tablet_app_directives_directory_path(app_directives_directory_path);
		}
		...
		if(device_type.equals("phone")){
			...
			gen_worker.setAndroid_phone_app_services_directory_path(app_services_directory_path);
		}else{
			app_services_directory_path = Paths.get(
					gen_worker.getAndroid_tablet_app_root_js_directory_path()
									.toString() + this.file_separator + tokens[0]);
			Files.createDirectory(app_services_directory_path);
			gen_worker.setAndroid_tablet_app_services_directory_path(app_services_directory_path);
		}
	}
