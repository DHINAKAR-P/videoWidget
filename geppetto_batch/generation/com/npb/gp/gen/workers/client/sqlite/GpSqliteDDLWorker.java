...
@Component("GpSqliteDDLWorker")
public class GpSqliteDDLWorker extends GpGenBaseWorker{
	private Path template_group_path;
	private GpSqliteDMLGenSupport ddl_gen_support;
	...
	private void set_up_paths_and_templates() {
		...
		//System.out.println("-----------root_code_template_location--------"+root_code_template_location);
		String server_rdbms_mysql_template_location = this.base_configs
				.get("client_mobile_sqlite_db_location").getValue();
		//System.out.println("---------server_rdbms_mysql_template_location-----------"+server_rdbms_mysql_template_location);
		...
		//System.out.println("---template_group_path---------"+this.template_group_path);
	}
	...
	private void write_db_creation_script(ST st) throws Exception{	
		...
		String the_path_string = server_root_path.toString()+ this.file_separator + "db_mobile_creation_script.sql"; //+"js"+ this.file_separator +"angular" + this.file_separator +"phone"+ this.file_separator 
		/*
				GpGenConstants.GEN_SERVER_DIRECTORY_NAME_PATH).getObject_value(); 
		String the_path_string = server_root_path.toString() + this.file_separator + "db_creation_script_mobile.sql";*/
		//System.out.println("---------the_path_string-------------"+the_path_string);
	...
	private void write_table_creation_scripts(ST st) throws Exception{
		Path server_root_path = (Path) this.derived_configs.get(
				GpGenConstants.GEN_CLIENT_DIRECTORY_NAME_PATH).getObject_value();
		//System.out.println("---------GEN_SERVER_DIRECTORY_NAME_PATH-----------"+server_root_path); 
		String the_path_string = server_root_path.toString()+ this.file_separator +"tables_db_mobile_script.sql";//+"js"+ this.file_separator +"angular" + this.file_separator +"phone"+ this.file_separator  

	/*GpGenConstants.GEN_SERVER_DIRECTORY_NAME_PATH).getObject_value(); 		
		String the_path_string = server_root_path.toString() + this.file_separator + "tables_db_script_mobile.sql";*/
				Path test_write_path = Paths.get(the_path_string);
		super.write_file(test_write_path, st);
	}
	...
	@Resource(name="GpSqliteDMLGenSupport")
	public void setDdl_gen_support(GpSqliteDMLGenSupport ddl_gen_support) {
		this.ddl_gen_support = ddl_gen_support;
	}
	
	public GpSqliteDMLGenSupport getDdl_gen_support() {
