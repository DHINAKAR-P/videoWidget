...
@Component("GpSpringBootControllerTakePhotoHandler")
public class GpSpringBootControllerTakePhotoHandler 
extends GpBaseSpringBootControllerVerbFunctionHandler
	...
	private GpControllerSpringBootVerbGenInfo gp_update(GpVerb verb, GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		GpControllerSpringBootVerbGenInfo update = the_update_handler.handle_implicit_verb(verb, activity,service_methods);
	...
	public void gp_declare_noun(GpVerb verb, GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		...
		String set_picture_method = null;
		for(GpNounAttribute noun:activity.getPrimary_noun().getNounattributes()){
			...
			if(noun.getSubtype().equals("Picture")){
				...
				set_picture_method ="set"+attr_name;
			}
			System.out.println("---set_picture_method----"+set_picture_method);
		}
		...
		if (!method_description.getReturn_parm().container) {
			if (method_description.getReturn_parm().name
				...
				the_dto.declare_noun = the_dto.return_type
						...
						"\n"+ "a_"+activity.getPrimary_noun().getName()+"."+ set_picture_method +"(uploadfile.getBytes())";
	...
	public void gp_service_call(GpVerb verb,GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		...
		/*SummaNoun testfile = new SummaNoun();   ///
		testfile.setProfilepic(uploadfile.getBytes());
		System.out.println("file in bytes---" + testfile.getProfilepic());

		SummaNoun filefinal = SummaTest_service.create_summanoun(testfile,super.getUser());
		filefinal.setProfilepic(null);
		//System.out.println("--filefinal-----" + filefinal.getProfilepic() + "---id---" + filefinal.getId());
		return filefinal ;*/
	...
	public void get_function_signiture(GpVerb verb,GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		...
		the_dto.request_map_value = "uploadFile";
		the_dto.reference_request_map_value = "uploadFile";
		the_dto.method_name = "uploadFile";
