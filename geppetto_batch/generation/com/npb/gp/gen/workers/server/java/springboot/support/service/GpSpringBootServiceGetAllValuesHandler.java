 ...
 */
@Component("GpSpringBootServiceGetAllValuesHandler")
public class GpSpringBootServiceGetAllValuesHandler extends
...
IGpSpringBootServiceVerbGenSupport {
	...
	public void get_function_signiture(GpVerb verb,GpActivity activity) 
		...
		the_dto.method_signiture = the_dto.signiture_helper.getDescription();
		the_dto.input_parms = "";
		// START SET UP EXCEPTION
		// service_exceptions =
		// (GpJavaException)method_sig_info.get("method_exception");
		int i = 0;
		for (String exc : the_dto.signiture_helper.getExceptions()) {
			if (i == 0) {
	...
	public void gp_declare_noun(GpVerb verb, GpActivity activity) throws Exception {
		GpJavaMethodDescription service_method_sig_info = this.the_dto.signiture_helper;
	...
	public void gp_service_call(GpVerb verb,GpActivity activity) throws Exception {
		...
		GpJavaMethodDescription dao_method_sig_info = dao_sigs
				.get(GpBaseVerbsConstants.GpGetAllValues).signiture_helper;
		...
		if(map_relations != null){
			//System.out.println("map_relations not null " + activity.getId());
			...
			if(rel_info != null){
				...
				for(GpChildRelationshipInfo child: childs){
					...
					post_dao += "for(" + activity.getPrimary_noun().getName() + " " + activity.getPrimary_noun().getName() + " : " + the_dto.return_reference + "){\n";  
					post_dao += "\t" + activity.getPrimary_noun().getName() + ".set" + child.getNoun().getName() 
							+ "("
					...
					for(GpDataType input_param : input_parms){
						...
						if(input_param.base_name.equals(noun_with_id_suffix)){
							post_dao += activity.getPrimary_noun().getName() + ".getId(),";
						}
						else{
							post_dao += "0,";
						}
