...
@Component("GpSpringBootDaoDeleteByParentIdHandler")
public class GpSpringBootDaoDeleteByParentIdHandler extends GpBaseSpringBootDaoVerbFunctionHandler
	...
	public void setDao_signiture_hlpr(GpSpringBootDaoVerbMethodSignitures dao_signiture_hlpr) {
	...
	@Override
	public GpSpringBootDaoVerbGenInfo handle_verb(GpVerb verb, GpActivity activity) throws Exception {
	...
	public GpSpringBootDaoVerbGenInfo handle_implicit_verb(GpVerb verb,
		...
		try {
			this.the_dto.signiture_helper = 
					this.dao_signiture_hlpr.delete_nound_by_parent_id_method_signiture(activity,this.the_worker.getRelationships_map());
	...
	public void gp_jpa_query(GpVerb verb, GpActivity activity) throws Exception {
		...
		ArrayList<GpBaseSqlDto> sql_stmts = this.the_worker.getSpring_boot_dml_worker()
				.get_dml_statements_for_activity(activity);
		for (GpBaseSqlDto stmt_dto : sql_stmts) {
			if (stmt_dto.verb_action_on_data.equals(GpBaseVerbsConstants.GpDeleteByParentId)) {
