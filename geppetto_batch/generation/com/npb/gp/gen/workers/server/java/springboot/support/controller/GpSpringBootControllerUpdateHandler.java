 ...
 */
@Component("GpSpringBootControllerUpdateHandler")
public class GpSpringBootControllerUpdateHandler extends 
                     GpBaseSpringBootControllerVerbFunctionHandler implements
                     IGpSpringBootControllerVerbGenSupport {
	
	String function_name = "update";
	private GpSpringBootControllerSearchForUpdateHandler the_search_for_update_handler;
	...
	public GpSpringBootControllerSearchForUpdateHandler getThe_search_for_update_handler() {
	...
	public void setThe_search_for_update_handler(
			GpSpringBootControllerSearchForUpdateHandler the_search_for_update_handler) {
	...
	private void handle_implicit_verbs(GpVerb verb, GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		...
		the_implicit_verbs.add(this.gp_search_for_update(verb, activity,service_methods));
		//
	}
	
	private GpControllerSpringBootVerbGenInfo gp_search_for_update(GpVerb verb, GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		GpControllerSpringBootVerbGenInfo search_for_update = the_search_for_update_handler.handle_implicit_verb(verb, activity,service_methods);
	...
	public void get_function_signiture(GpVerb verb,GpActivity activity,
		...
		the_dto.request_map_value = "update_" + activity.getPrimary_noun().getName();
		the_dto.reference_request_map_value = "update_" + activity.getPrimary_noun().getName();
	...
	public void gp_declare_noun(GpVerb verb, GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		...
		if (!method_description.getReturn_parm().container) {
			if (method_description.getReturn_parm().name
				...
				the_dto.declare_noun = the_dto.return_type + " " + "a_"
						+ activity.getPrimary_noun().getName();
			}
		} else {
	...
	public GpControllerSpringBootVerbGenInfo handle_implicit_verb(GpVerb verb, GpActivity activity,
			HashMap<String, GpServiceVerbGenInfo> service_methods) { 
