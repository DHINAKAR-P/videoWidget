 ...
 */
@Component("GpSpringBootServiceDeleteHandler")
public class GpSpringBootServiceDeleteHandler extends
	...
	public void gp_service_call(GpVerb verb,GpActivity activity	) throws Exception {
		...
		GpJavaMethodDescription dao_method_sig_info = dao_sigs
				.get(GpBaseVerbsConstants.GpDelete).signiture_helper;
		...
		if(map_relations != null){
			...
			if(rel_info != null){
				...
				for(GpChildRelationshipInfo child: childs){
					GpJavaMethodDescription java_method_descript = this.verb_signiture_hlpr.delete_nound_by_parent_id_method_signiture(child.getActivity(), this.the_worker.getRelationships_map());
					post_dao += child.getActivity().getName() + "Service." 
