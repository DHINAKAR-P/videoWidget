...
@Component("GpSpringBootControllerDeleteByParentIdHandler")
public class GpSpringBootControllerDeleteByParentIdHandler extends GpBaseSpringBootControllerVerbFunctionHandler
	...
	@Override
	public GpControllerSpringBootVerbGenInfo handle_verb(GpVerb verb, GpActivity activity,
	...
	public void get_function_signiture(GpVerb verb, GpActivity activity,
		...
		the_dto.method_notes = "delete by parent" + " of " + activity.getPrimary_noun().getName();
		...
		the_dto.request_map_value = "delete_" + activity.getPrimary_noun().getName() + "_by_parent_id";
		the_dto.reference_request_map_value = "delete_" + activity.getPrimary_noun().getName() + "_by_parent_id";
		the_dto.method_name = "delete_" + activity.getPrimary_noun().getName() + "_by_parent_id";
		...
		GpJavaMethodDescription method_description = service_methods.get(GpBaseVerbsConstants.GpDeleteByParentId).signiture_helper;
	...
	@Override
	public void gp_start(GpVerb verb, GpActivity activity, HashMap<String, GpServiceVerbGenInfo> service_methods)
			throws Exception {
	...
	@Override
	public void gp_service_call(GpVerb verb, GpActivity activity, HashMap<String, GpServiceVerbGenInfo> service_methods)
			throws Exception {
		GpJavaMethodDescription method_description = service_methods.get(GpBaseVerbsConstants.GpDeleteByParentId).signiture_helper;
		// START - SERVICE CALL
		int i = 0;
		if (method_description.getInput_parms().size() == 0) {
			the_dto.service_call = activity.getPrimary_noun().getName() + "_list = " + activity.getName() + "_"
					+ "service" + "." + method_description.getName() + "()";
		} else {
			the_dto.service_call = "service_return_msg" + " = " + activity.getName() + "_" + "service" + "."
			...
			for (GpDataType parm : method_description.getInput_parms()) {
				...
				if (!parm.container) {
					if (parm.name.equals("GpLong")) {
						parm_string = parm_string + parm.base_name;
	...
	@Override
	public void gp_return(GpVerb verb, GpActivity activity, HashMap<String, GpServiceVerbGenInfo> service_methods)
			throws Exception {
	...
	public void gp_declare_noun(GpVerb verb, GpActivity activity, HashMap<String, GpServiceVerbGenInfo> service_methods)
		...
		if (!method_description.getReturn_parm().container) {
			if (method_description.getReturn_parm().name.equals("GpPrimaryNoun")) {
				the_dto.return_type = activity.getPrimary_noun().getName();

				the_dto.declare_noun = the_dto.return_type + " " + "the_" + activity.getPrimary_noun().getName()
						+ " = new " + the_dto.return_type + "()";
			} else if (method_description.getReturn_parm().name.equals("GpString")) {
				the_dto.declare_noun = "String service_return_msg = \"\"";
			...
			if (method_description.getReturn_parm().name.equals("GpArrayList")) {
				...
				if (method_description.getReturn_parm().base_name.equals("GpPrimaryNoun")) {
					the_dto.return_type = the_dto.return_type + activity.getPrimary_noun().getName() + ">";
				}
				the_dto.declare_noun = the_dto.return_type + " " + activity.getPrimary_noun().getName() + "_list"
						+ " = new " + the_dto.return_type + "()";
			}
		}
