 ...
 */
@Component("GpSpringBootControllerDeleteHandler")
public class GpSpringBootControllerDeleteHandler extends
		...
		try {
			...
			ArrayList<GpMicroFlow> the_micro_flow = super.getMicro_flow_dao()
					...
					/*for (GpMicroFlow gpMicroFlow : the_micro_flow) {
						System.out.println("spring controller delete : " + gpMicroFlow.toString());
					}*/
	...
	public void get_function_signiture(GpVerb verb,GpActivity activity,HashMap<String, GpServiceVerbGenInfo> service_methods) throws Exception {
		...
		the_dto.request_map_value = "delete_" + activity.getPrimary_noun().getName()+ "/{"+ activity.getPrimary_noun().getName() + "_id" +"}";
		the_dto.reference_request_map_value = "delete_" + activity.getPrimary_noun().getName();
