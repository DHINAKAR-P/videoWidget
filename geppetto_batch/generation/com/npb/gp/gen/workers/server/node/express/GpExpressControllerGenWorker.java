...
import com.npb.gp.gen.workers.server.node.express.support.controller.GpExpressControllerGenSupport;

@Component("GpExpressControllerGenWorker")
public class GpExpressControllerGenWorker extends GpGenNodeServerExpressBaseWorker{
	...
	private void set_up_paths_and_templates() {
		...
		String server_nodejs_express_dao_file_template_location = this.base_configs
				.get("server_nodejs_express_controller_template_location").getValue();
		String[] tokens = this.tokenize_string(
				server_nodejs_express_dao_file_template_location, null);
		String config_name;
		config_name = this.build_name_from_tokens(tokens) + "_controller_file" + GpGenConstants.DEFAULT_CONFIG_PATH_EXTENSION;	
		...
		String template_name =  this.base_configs.get(
				"server_nodejs_express_controller_template_name").getValue();
	...
	public void generate_code_by_activity(GpActivity activity, GpProject the_project,
		...
		Path controllers_path = getGen_service().getThe_directory_worker().getServer_controllers_root_path();
		ST st = super.read_template_group(this.template_group_path, "output");
		st.add("controllers", the_gen_support.get_the_methods(activity));
		st.add("activity_name", activity.getName());
		String the_path_string = controllers_path + this.file_separator + activity.getName() + "Controller" + file_extension;
	...
	@Resource(name="GpExpressControllerGenSupport")
